/**
 * Der rechnende Teil des Sandsackbedarfs — **ohne Karte und ohne React**.
 *
 * Getrennt von `sandsack.ts`, weil es zwei Aufrufer gibt: die Dammlinie auf der
 * Karte (dort ist Leaflet ohnehin geladen) und den MCP-Server, der im
 * Node-Prozess läuft. Ein Import der Fassade zöge über `connectionDisplayPositions`
 * das gesamte Leaflet mit — im Server bricht das beim Laden ab, weil es dort
 * kein `window` gibt.
 */
