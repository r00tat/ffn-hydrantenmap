...
locals {
  ...
  # Queue-Namen erst nach sieben Tagen wieder freigibt. In dev hat genau das
  # die Pipeline lahmgelegt; hier steht die Kennung, damit derselbe Stolper-
  # stein nicht in prod wartet. Sie liegt im State und bleibt über Applies
  # hinweg stehen; ist ein Name doch einmal verbrannt, holt
  # `tofu apply -replace` einen neuen, statt eine Woche zu warten. Der Name
  # ist frei wählbar, weil ihn niemand von außen kennt: der Dienst liest den
  # Pfad aus der Umgebung.
  ueberwachung_queue = "atemschutz-ueberwachung-${random_id.ueberwachung_queue.hex}"
  ...
  ueberwachung_invoker = "fahrtenbuch-report-invoker@${var.project}.iam.gserviceaccount.com"
}
...
module "cloud_scheduler" {
  ...
  # einsatz.ffnd.at, und `getBaseUrl()` leitet die erwartete Audience aus dem
