...
class VsfrTest {
    ...
    @Test fun `PLAY_SIGNAL value and format`() {
        ...
        assertSame(VsfrFormat.ThreeXByte, Vsfr.PLAY_SIGNAL.format)
    }
    ...
    @Test fun `SYS_MCU_VREF format is signed i`() {
        ...
        assertSame(VsfrFormat.I32, Vsfr.SYS_MCU_VREF.format)
    }

    @Test fun `ACC_X format`() {
        assertEquals(0x8025, Vsfr.ACC_X.value)
        assertSame(VsfrFormat.TwoXShort, Vsfr.ACC_X.format)
    }
